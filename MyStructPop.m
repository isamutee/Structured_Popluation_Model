...
%             num = 1:length(app.CListBox.String);
            ...
            for i = 1:3
                axg(i).Value = max(axg(i).Value-1, 1);
                axg(i).String = app.CListBox.String;
            end
        app.changed;
        app.SumI;
        ...
        function TChangeButtonPushed(app, ~, ~)
            value = str2double(app.TChangeText.String);
             ...
             if ~isempty(app.TrajectoryListBox.String)
                 ...
                 for i = 1:length(Tags)
                     l = Tags(i);
                     app.PPlot.Trj(l).XData = app.Cate(app.PPlot.xa).Values(l,1:app.ModInfo.T);
                     app.PPlot.Trj(l).YData = app.Cate(app.PPlot.ya).Values(l,1:app.ModInfo.T);
                 end
                 app.refreshTP;
             end
        ...
        function TrajectoryListBoxValueChanged(app, ~, ~)
            [~, Tags] = app.getTag('t');        
            prev = app.TrajectoryListBox.Value;
            value = str2double(app.TrajectoryListBox.String(prev));
            set(app.PPlot.Trj(Tags), 'Marker', 'none');            
        ...
        function DeleteCurrentTrajectoryButtonPushed(app, ~, ~)                        
            prev = app.TrajectoryListBox.Value;
            value = str2double(app.TrajectoryListBox.String(app.TrajectoryListBox.Value));
...
%             [~, Tags] = app.getTag('t');
            if isempty(app.TrajectoryListBox.String)
                ...
                app.TrajectoryListBox.Value = max(1, prev-1);
...
%                          ylabel(app.PhasePlaneAx, app.Cate(app.PPlot.ya).Name)
    ...
    methods
        ...
        function refreshPara(app)
            PTags = [app.Para(~strcmp({app.Para.Symbol}, '')).Tag];
            Pnum = length(PTags);
            app.PListBox.String =  sort({app.Para(PTags).Symbol});
...
%                 num = 1:l;
        ...
        Ctags = [app.Cate(~strcmp({app.Cate.Symbol}, '')).Tag];
        l = str2double(app.TrajectoryListBox.String(app.TrajectoryListBox.Value));
        app.TPlot.Lns.delete;
            for i = 1:length(Ctags)
    ...
    methods
        ...
        function createExport(app)
            ...
            for i = 1:length(CTags)
                ...
                fprintf(fil, '%%        %.0f. %s; \n', [1:length(ATags);string({app.Interact(ATags).Disp})]);
                fprintf(fil, '%%\n');
            end
    ...
    methods
        ...
        function [Tag, Tags] = getTag(app, s, obj)
            switch s
                ...
                case 'a'
                CnowTag = app.getTag('c', app.CListBox);      
                if isempty(app.ActionList.String)
                    app.ActionAddButtonPushed;                    
                end          
